...
async function main() {
  ...
  const calendarTitle = data.calendarTitle || "カレンダー";

  // 1. ヘッダー
  svg += `<text x="${marginX + 4}" y="${marginY + 26}" font-size="26" font-weight="900" fill="#000000" font-family="'Helvetica Neue', Arial, sans-serif">${year}年 ${month}月</text>`;
  svg += `<text x="${width - marginX - 4}" y="${marginY + 25}" font-size="14" font-weight="bold" fill="#444444" text-anchor="end" font-family="sans-serif">${escapeXml(calendarTitle)}</text>`;
  ...
  for (let week = 0; week < 6; week++) {
    ...
    rawEvents.forEach(ev => {
      if (!ev || !ev.startDateStr) return;
      const sIdx = getDayIndex(ev.startDateStr);
      const eIdx = ev.endDateStr ? getDayIndex(ev.endDateStr) : sIdx;
    ...
    weekEvents.forEach(ev => {
      ...
      if (assignedSlot !== -1) {
        ...
        if (ev.isStartOfWeekOrEvent) {
          ...
          const maxChars = Math.floor(ev.span * 6.5);
          const trimmed = displayTitle.length > maxChars ? displayTitle.slice(0, maxChars - 1) + '…' : displayTitle;

          svg += `<text x="${textX}" y="${textY}" font-size="11" font-weight="bold" fill="#000000" font-family="sans-serif">${escapeXml(trimmed)}</text>`;
        }
      }
    });
  }
